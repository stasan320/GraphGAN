...
struct DCGANGeneratorImpl :
    ...
    DCGANGeneratorImpl(int SizeInputG) :
        ...
        conv4(nn::ConvTranspose2dOptions(64, 3, 4).stride(2).padding(1).bias(false))
    {
...
public:
    ...
    Example get(size_t index) {
        std::string path = options.datasetPath + std::to_string(index % 10) + "\\" + data[index].first;
        ...
        auto tdata = torch::cat({ R, G, B }).view({ 3, image_size, image_size }).to(torch::kFloat);
...
int main() {
    ...
    nn::Sequential discriminator(
        nn::Conv2d(nn::Conv2dOptions(3, 64, 4).stride(2).padding(1).bias(false)),
